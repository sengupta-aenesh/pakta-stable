...
export default function InteractiveTemplateEditor({
  ...
  // Simple text position finder for template risk highlighting
  const findSimpleTextPosition = useCallback((text: string, clause: string): TextPosition => {
    ...
    const directIndex = text.indexOf(clause)
    if (directIndex !== -1) {
      return { start: directIndex, end: directIndex + clause.length }
    }
    
    // Strategy 2: Case insensitive match
    const lowerText = text.toLowerCase()
    const lowerClause = clause.toLowerCase()
    const caseInsensitiveIndex = lowerText.indexOf(lowerClause)
    if (caseInsensitiveIndex !== -1) {
      return { start: caseInsensitiveIndex, end: caseInsensitiveIndex + clause.length }
    }
    
    // Strategy 3: Find best matching substring
    ...
    if (clauseWords.length >= 2) {
      ...
      clauseWords.forEach(word => {
        ...
        if (wordIndex !== -1) {
          const score = word.length / clause.length
          if (score > bestMatch.score) {
            bestMatch = {
              start: wordIndex,
              end: Math.min(wordIndex + clause.length, text.length),
              score: score
            }
          }
        }
